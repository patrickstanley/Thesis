...
A small section of test coupon was placed in a crucible suspended from the balance and heated to \temp{650} at \temp{10}/min, the environment was switched from 50 sccm of 21\% O\textsubscript{2} in
...
\po2{} measurements were taken using a calibrated YSZ
sensor placed immediately before the sample.
Samples were cooled at
\temp{10}/min in the desired atmosphere.
...
After 20 minutes, the temperature would be increased to the desired set-point of \temp{650} at a ramp rate of \temp{10}/min and the gas would be switched first to N\textsubscript{2} to flush the chamber, then to humidified argon containing 3\% H\textsubscript{2}.
...
Chi-squared values, indicating the divergence of the fit from the measured data for each sample set, were calculated for the two fitting methods and are displayed in Table 1.
...
The deviance from the fit curve for the fixed η method is quite large relative to the variable η fit for both sample sets tested at \temp{650}.
...
\begin{figure}
    \includegraphics{GDCPoreSEM.jpg}
...
\begin{figure}
    ...
    \caption{Thermogravimetric analysis curves for Ni-GDC/GDC half-cells showing mass loss over time at temperatures ranging from \temp{550} to \temp{750} in 3\% H\textsubscript{2} 3\% H\textsubscript{2}O balance N\textsubscript{2}, 50sccm flow.}
...
For TGA analysis, a piece of a half cell was heated to \temp{650} with a
\temp{10}/min ramp rate and reduced.
...
\begin{figure}
    \includegraphics[width=0.6\textwidth]{Rereduction.jpg}
...
\begin{figure}
    \includegraphics[width=0.6\textwidth]{ReductionMethod.jpg}
