...
\section{Solid-Oxide Fuel Cell Operation}
    ...
    \begin{figure}[p]
      \centering
      \includegraphics[width=0.75\textwidth]{sofc.png}
    ...
    \begin{figure}[p]
      \centering
      \includegraphics[width=0.75\textwidth]{tpb.png}
    ...
    Historically, \glspl{sofc} operated near \temp{1000}, but advances in materials have allowed cells to decrease to the intermediate temperature range (\temp{650} to \temp{800}) and now even further to the low temperature range (below \temp{650}).\cite{Wachsman2011a}
...
\section{Solid-Oxide Fuel Cell Materials}
    ...
    \Gls{ysz} is very stable, has a high strength, has a ratio of ionic conductivity to total conductivity (transference number) of approximately 1, and good conductivity at temperatures in excess of \temp{800} even in very low \po2{}s.
    ...
    Other rare-earth doped cerias have been used as electrolytes, but \gls{gdc} tends to outperform them under a variety of conditions.%addcite
    ...
    Under conditions of high temperature or very low \po2{}, \gls{gdc} will become electrically conductive and start to leak electrons through the electrolyte, reducing cell performance.
    ...
    Additionally, nickel is prone to poisoning from sulfur contaminants in the fuel, so alternative anodes are of interest for use in \glspl{sofc}.%addcite

    Alternative metal-ceramic systems have been researched to serve the function of the anode.
    These have included copper, cobalt, and platinum systems, but each suffer from a significant drawback such as coking, long-term performance degradation or cost.%addcite
    A different approach is to use an all-ceramic anode, made of a \gls{miec} where both ionic and electronic conductions occur simultaneously, such as such as \ce{La_{\hbox{1--x}}Sr_{x}Cr_{\hbox{1--y}}Mn_{y}O_{3}}, \ce{Sr_{0.94}Ti_{0.9}Nb_{0.1}O_{3-\delta}}, \ce{Ba_{0.98}La_{0.02}SnO_3}, or \ce{Sr_{2}MgMoO_{6}}.\cite{Goodenough2007,Zha2005,Primdahl2001,Hussain2013,MohammedHussain2012,Hussain2016,Huang2006}

    Perovskite structures have yielded a number of \glspl{miec} with potential uses as electrode materials. \cite{Yamamoto1987,Anderson1992,Ishihara2009}
    \gls{smm} and its related family of materials (\ce{Sr_2MMoO_6}, where M is a transition metal dopant) yield good conductivities and catalytic actives.\cite{Huang2009}
    ...
    \gls{sfcm} is a recently developed double perovskite \gls{miec} with multiple dopants on the B-site with a high conductivity of \SI{30}{S\per\centi\meter} at \temp{650}.\cite{Pan,Hussaina,Hussain}
    ...
    Outside of the cell itself, several materials are needed to create a functioning stack.
    ...
    In intermediate and high temperature stacks, expensive metal alloys such as Inconel must be used to survive the extreme heat.%addcite
    ...
    At lower temperatures new options are available, such as \gls{ysz} felt, vermiculite or mica gaskets.%addcite
...
\section{Mechanical Properties}
    ...
    The first external stress applied to the cell, and the one most likely to result in failure, is the  sealing of an individual cell into a stack, creating a gas tight seal between the anode and cathode sides.
...
\section{Summary of Contributions}%needs a better name
    ...
    Most of the previous research into the mechanical properties of \glspl{sofc} has focused on \gls{ysz} based devices.\cite{Roa2011,Yu2007,Nakajo2012}
